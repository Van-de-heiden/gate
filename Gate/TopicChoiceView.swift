        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    Text("Zwei zufällige Themen. Du wählst eines und arbeitest es durch. Die Aufgaben unterwegs zählen mit.")
                    ...
                    if let offer = learning.choice?.offer, let catalog = learning.catalog {
                        ForEach(offer.topicIDs, id: \.self) { id in
                            if let topic = catalog.topic(id) {
                                let chapters = catalog.chapters(in: id)
                                ...
                                VStack(alignment: .leading, spacing: 16) {
                                    HStack {
                                        ...
                                        Spacer()
                                        Text(topic.format ?? "Entdeckung").font(.caption.weight(.semibold))
                                    }
                                    ...
                                    Button { learning.choose(id) } label: {
                                        HStack { Text("Dieses Thema lernen"); Spacer(); Image(systemName: "arrow.right") }
